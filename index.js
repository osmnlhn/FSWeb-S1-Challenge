...
function indekseGoreFenomen(arr,index) {
  let fenomen=arr[index]
  let result2= index +'indekste bulunan fenomen'+fenomen.profile;
  let result = `${index}. indekste bulunan fenomen: ${fenomen.profile}`;
 return result;
...
function fenomenGonderimSayisi(arr,profileName){
  ...
  for (let i = 0; i < arr.length; i++) {
    const fenomen = arr[i];
    if (fenomen.profile===profileName) {
...
function platformaGoreCokGonderiYapanFenomen(arr,platforName){
  let maxValue= -1;
  let fenomenName="";

  for (let i = 0; i < arr.length; i++) {
    const fenomen = arr[i];
   if (fenomen.platform === platforName) {
    if (fenomen.posts > maxValue) {
      maxValue=fenomen.posts;
      fenomenName=fenomen.profile;
